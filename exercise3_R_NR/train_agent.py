...
def process_actions(y):
    ...
    cleaned = np.where(stacked.sum(axis=1, keepdims=True) == 1, stacked, np.array([1, 0, 0, 0, 0])) #this should get broadcast
...
def rebalance_actions(x, y):
    ...
    y_sample_probs = y_action_sample_probs[y_actions] # each row's sample probability
    
    y_samples_required = np.int(np.round(1 / y_sample_probs.mean(), decimals=0))
...
def add_horizontal_flip(x, y):
    ...
    x_cat, y_cat = np.concatenate((x, x_flip), axis=0), np.concatenate((y, y_flip), axis=0)
...
def create_a_model(X_shape, y_shape, use_3d, num_filters,
...
def train_a_model(name, do_augmentation, history_length, use_3d,
                  num_filters, num_flat_units, drop_prob,
    ...
    if do_augmentation:
        ...
        X_train_ha, y_train_a = add_horizontal_flip(*rebalance_actions(X_train_h, y_train))
    else:
    ...
    print("\t... creating model " + str(X_train_ha.shape) + "->" + str(y_train_a.shape))
