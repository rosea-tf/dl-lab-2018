...
def results_dump(path, name, lr, filter_size, num_filters, batch_size, learning_curve):
        results = dict()
        results["name"] = name
        results["lr"] = lr
...
class Kerosey(object):
    ...
    class Conv2D(Layer):
        def __init__(self, last_layer, filter_size, num_filters, stride):
            ...
            self.weights = tf.get_variable('w' + str(id(self)),
                                           shape=(filter_size,
                                                  filter_size,
                                                  last_layer.shape[-1],
                                                  num_filters),
                                           initializer=tf.contrib.layers.xavier_initializer())
            self.bias = tf.get_variable('b' + str(id(self)),
                                        shape=(num_filters,),
                                        initializer=tf.contrib.layers.xavier_initializer())
    ...
    class Flatten(Layer):
        def __init__(self, last_layer):
            ...
            self.shape = (last_layer.shape[0], reduce(
                lambda x, y: x * y, last_layer.shape[1:], 1))
    ...
    class LayeredModel(object):
        ...
        def compile(self):
            self.predictor = reduce(lambda f, g: lambda x: g(
                f(x)), [l.func for l in self.layers], lambda x: x)
        ...
        def accuracy(self):
            return tf.reduce_mean(
                tf.cast(tf.equal(tf.argmax(self.tfp_y, 1), tf.argmax(self.prediction(), 1)), tf.float32))
    
        def loss(self):
            tf_losses = tf.losses.softmax_cross_entropy(self.tfp_y, self.prediction())
        ...
        def crunch(self, x, y, batch_size, results, collect_stats=False):
            ...
            for x_batch, y_batch in make_batches(x, y, batch_size):
                ...
                r = self.session.run(results, feed_dict={self.tfp_x: x_batch, self.tfp_y: y_batch})
                
                if collect_stats:
                    if rr is None:
                        rr = [v * x_batch.shape[0] for v in r]
                    else:
                        rr = [np.add(vv, v * x_batch.shape[0]) for v, vv in zip(r, rr)]
        ...
        def train(self, x_train, y_train, x_valid, y_valid, lr, epochs, batch_size):
            
            optimiser = tf.train.GradientDescentOptimizer(learning_rate=lr).minimize(self.loss())
            ...
            for i in range(epochs):
                ...
                train_loss, train_acc = self.crunch(x_train, y_train, 1024, [self.loss(), self.accuracy()], True)
                
                valid_acc, = self.crunch(x_valid, y_valid, 1024, [self.accuracy(),], True)
        ...
        def test(self, x_test, y_test, batch_size=1024):
            self.test_acc = self.crunch(x_test, y_test, batch_size, [self.accuracy()], True)
            print ("Test Accuracy {:4f}".format(self.test_acc))
            pass
            
        def save(self, directory):
            save_path = tf.train.Saver().save(self.session, os.path.join('.', directory, 'model.ckpt'))
            print("Model saved in path: %s" % save_path)
            pass

        def restore(self, directory):
            save_path = tf.train.Saver().restore(self.session, os.path.join('.', directory, 'model.ckpt'))
...
if __name__ == '__main__':
    ...
    x_train, y_train, x_valid, y_valid, x_test, y_test = mnist(os.path.expanduser(os.sep.join(["~","data"])), recs=500)
    ...
    for i in range(1, 5):
        ...
        krmodel.train(x_train, y_train, x_valid, y_valid, lr, epochs, batch_size)
        
        learning_curve = [1 - a for a in krmodel.valid_accs]
        
        results_dump(results_path, name, lr, filter_size, num_filters, batch_size, learning_curve)
